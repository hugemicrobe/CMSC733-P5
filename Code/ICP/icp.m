...
validSampling = {'all', 'random', 'nspace'};
inp.addParameter('Sampling', 'all', @(x)any(strcmpi(x,validSampling)));

inp.addParameter('Boundary', [], @(x)size(x,1) == 1);

inp.addParameter('EdgeRejection', false, @(x)islogical(x));

inp.addParameter('Extrapolation', false, @(x)islogical(x));
...
switch arg.Sampling
    case 'all'
        % do nothing
    case 'random'        
        nSamples = round(Np * 0.6);
        ind = randperm(Np, nSamples);
        
        p = p(:, ind);
        q = q(:, ind);        
    case 'nspace'    
        % bucket arg.Normals
        C = kmeans(arg.Normals', 250, 'Distance', 'cosine');  
end
